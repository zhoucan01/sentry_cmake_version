 ...
 * @author NeoZeng neozng1@hnu.edu.cn (original)
 * @brief 伪pubsub机制，对应用之间的通信进行隔离
 *        适配版本：添加 FreeRTOS 互斥锁支持抢占式多任务
 * @version 0.2
 * @date 2022-11-30 (original) / 2026-06-29 (adapted)
 *
...
#define MAX_TOPIC_COUNT 20    // 最多支持的话题数量（链表实现，此值为软限制）
#define QUEUE_SIZE 1          // 每个订阅者的队列深度
...
/**
 * @brief 初始化消息中心，创建互斥锁
 *        必须在 FreeRTOS 调度器启动前调用（在 main() 中 MX_FREERTOS_Init() 之前）
 */
