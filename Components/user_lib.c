...
uint8_t MedianFilter_Update(MedianFilter *filter, uint8_t newValue) {
    filter->window[filter->index] = newValue;
    filter->index = (filter->index + 1) % (FILTER_SIZE * FILTER_SIZE);
    
    // 计算中值
    uint8_t sorted[FILTER_SIZE * FILTER_SIZE];
    for (int i = 0; i < FILTER_SIZE * FILTER_SIZE; i++) {
    ...
    return sorted[(FILTER_SIZE * FILTER_SIZE) / 2]; // 返回中位数
}
...
uint8_t MeanFilter_Update(MeanFilter *filter, uint8_t newValue) {
    // 更新窗口
    filter->sum -= filter->values[filter->index]; // 减去旧值
    filter->values[filter->index] = newValue; // 存入新值
    filter->sum += newValue; // 加上新值
    filter->index = (filter->index + 1) % (FILTER_SIZE * FILTER_SIZE); // 更新索引

    // 如果填满了窗口，更新计数
    if (filter->count < FILTER_SIZE * FILTER_SIZE) {
        filter->count++;
    }

    // 返回均值
    return filter->sum / filter->count; // 返回当前均值
}