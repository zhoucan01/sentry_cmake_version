#ifndef __BSP_SIN_COS_ATAN_SQRT_H
#define __BSP_SIN_COS_ATAN_SQRT_H

#define REAL float
#define TAN_MAP_RES 0.003921569 /* (smallest non-zero 
value in table) */
...
REAL fast_atan2(REAL y, REAL x) {
	...
	if ((y == 0.0) && (x == 0.0))
		...
		/* best approximated with the argument 
		itself... */
		if (z < TAN_MAP_RES)
		base_angle = z;
		else {
		...
		/* add or subtract table value from base angle based on 
		quadrant */
		base_angle = fast_atan_table[index];
		base_angle +=
		(fast_atan_table[index + 1] - fast_atan_table[index]) * 
		alpha;
		} 
